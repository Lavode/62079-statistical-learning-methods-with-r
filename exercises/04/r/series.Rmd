...
Clearly variables such as the model or vendor name are not applicable to predict performance in a linear model. Further, using PRP to predict itself would be an adventurous choice. Lastly ERP should not be used, as this field is the result of a linear prediction of PRP, based on the available predictor values, done by the paper's authors.
